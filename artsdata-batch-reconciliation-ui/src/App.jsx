...
const App = ({ config }) => {
  ...
  if (!showAll) {
    itemsAfterShowAllFilter = reconciledItems.filter(item => {
      ...
      // Hide reconciled entities. linkedTo is only set for entities actually linked in
      // Artsdata (pre-reconciled, user-linked, or minted). An unasserted sameAs claim has
      // linkedTo = null and stays visible so it can be judged.
